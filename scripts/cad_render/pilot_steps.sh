...
$R --out "$OUT/02-10" --note "$NOTE" \
  ...
  --subtitle-a 'One of four. The stop clips over the free top end of the MGN9 rail.' \
  --subtitle-b 'Same parts in the frame; rest of the machine ghosted at 20%.' \
  --context-scale 1.9

# --- 05.18  second titanium Y backer ----------------------------------------
$R --out "$OUT/05-18" --note "$NOTE" --elev -24 \
  --select 'Gantry Extrusions:1/HFSB5-2020-350[^/]*:1/=>'                        --label 'C extrusion (Y beam) x2' \
  --select 'Gantry Extrusions:1/HFSB5-2020-350[^/]*:1/MGN9 - 300mm[^/]*:1/=>'    --label 'MGN9 rail - this face' \
  --box    '313,8,88,331,326,91;-77,8,88,-59,326,91:titanium Y backer - opposite face (schematic)' \
  --title 'Step 05.18 - both Y backers go on the face opposite the rail' \
  --subtitle-a 'Viewed from below the gantry so the rail face is visible. Backers shown schematically.' \
  --subtitle-b 'Both Y beams in the gantry, seen from below; rest ghosted at 20%.' \
  --context-scale 1.3
