...
**Status 2026-09-06 (early morning, resume complete):** manual complete, reviewed (six tracks, review/2026-09-05/), Fable first-time-builder reviews F1–F9 and verification re-reads G1–G8 all done and applied (review/2026-09-06/); no open blockers. Live site: Prusa-style step pages (docs/manual/steps/, generated), overview grids, progress by step, Tonight planner (30-min sessions), Source line + "What you're looking at" on every step, Stealthburner/LDO/CAD/diagram/part-render imagery, per-bin QR labels and 27 plate sorting diagrams drawn from the committed 3MFs (25-bin scheme in slicer/bins.py; Alex may re-arrange plates in the PrusaSlicer GUI — diagrams/estimates derive from the 3MFs via `slicer/build_plates.py --from-3mf`). Guard rails: scripts/lint_manual.py, slicer/check_docs.py (incl. index/SVG/README figure consistency), CI build+deploy, weekly drift job. Rulings live in memory (`fable-review-2026-09-06`, `plate-diagrams-and-bins`, `manual-format-step-pages-no-photos`). Deferred until the printer runs: maintenance + first-prints chapters. Bench-verify items are marked `(verify on bench)` in-chapter.
