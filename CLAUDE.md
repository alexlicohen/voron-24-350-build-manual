...
**Status 2026-09-05 (night, pushed as dcc5fa7):** manual complete, reviewed (six tracks, review/2026-09-05/), and through the improvements wave — every step has a `Source:` line (pinned commits) and a `**What you're looking at:**` description; `Pause:` stopping points (~30-min sessions) feed the generated Tonight view (docs/manual/00-tonight.md); new chapters 00a mains safety, 15 troubleshooting, 16 glossary; Stealthburner pages, LDO/Nitehawk images (attributed, Alex's ruling), per-part STL renders (assets/parts), explanatory SVG diagrams (assets/diagrams), CAD-render pilot (assets/cad/PILOT.md); per-plate PrusaSlicer 2.9.6 projects + real estimates (slicer/); progress checkboxes/resume, jump bar, printables (checklists, QR bin labels), keep-awake, bench-photo ingest (scripts/ingest_photos.py); sources.yml + weekly drift job + issue/PR templates; lint + CI deploy. Skipped by Alex's call: chapter splitting, LDO-web drift diffing, mass Check-shortening, mods roadmap; no PrusaSlicer 3.0 migration. 2026-09-06 (night, expiring Fable credits): nine Fable read-through/adversarial reviews (first-time + second-time builder personas, pin/command trace) → findings F1–F9 in review/2026-09-06/, ALL applied by eight Fable fix agents (rulings in memory `fable-review-2026-09-06`); Prusa-style step pages live. Re-read verification wave (G1–G8) was cut short by the session usage limit (resets 12:40 AM ET): G1 (Ch 00/00a/01) completed — 33/33 fixes verified, quick items applied (pushed 1287875), four seams listed in memory `fable-review-2026-09-06` for next session; G7 (Ch 12–14) completed — 4 MAJOR found, two applied (no M84 with gantry up; PrusaSlicer temperature-emit untick), two pending (idle timeout before soaks; 06b hot/cold wording) in memory; G4 (Ch 06–07) completed — BLOCKER OPEN: Ch 07 Steps 07.17–07.22 (B-belt route) are the reverse traversal and not threadable; interim ⚠ banner in Ch 07 tells the builder to thread B from manual p.131/136/138 until a fix agent re-orders them (+ diagram 03 B panel, + the "hot, chamber closed" tension sentence in 7 places); G3 (Ch 04–05) completed — rail-band seam closed; OPEN MAJOR: pulley-jig geometry in 04.24/04.33 (gauge surfaces are the tab undersides, 16.6/6.6 mm) — see memory; G2 (Ch 02–03) was still running; 08–09, 10–11 and print/index re-reads must be re-run. Bench-verify items are marked `(verify on bench)` in-chapter. Video source: Alex's YouTube playlist (memory `youtube-build-playlist`) → timestamped links per step in docs/manual/assets/video/MANIFEST.md; no stills without his OK.
